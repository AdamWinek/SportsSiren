...
function Scoreboard(props) {
    ...
    useEffect(() => {
        let getGame = async function () {
            ...
            try {
                let result = await axios.get(methodUrl + 'gameInDBNBA/b4afe94b-65de-4826-ba4c-8a112b1367f1', {
                })
                console.log(result)
                setGame(result.data)
        ...
        return (
            <div style={{ width: '800px', height: '400px', display: 'flex', justifyContent: 'space-between' }}>
                <div>
                    ...
                    <img style={{ width: '200px', height: '200px' }} src={game ? game.homeLogoUrl : 'none'}></img>
                </div>
                <div>
                    <h1>{game ? game.awayTeam : 'loading'}</h1>
                    <h3>{game ? game.awayPoints : 'loading'}</h3>
                    <img style={{ width: '200px', height: '200px' }} src={game ? game.awayLogoUrl : 'none'}></img>
